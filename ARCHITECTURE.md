...
│   │                 │  │                 │  │               │  │
│   │ • 1B Supply     │  │ • createQuest() │  │ • Guest       │  │
│   │ • ERC20+Votes   │  │ • submitProof() │  │ • Citizen     │  │
...
|---|-----------|-------------|
| 1 | **No Coercion** | Sidecar requires explicit user consent for sensitive operations |
...
|------|-------------|--------|-------|
| **Guest** | 0 | Basic protection, view ledger, request citizenship | Visitor Pass |
| **Citizen** | 50 | Earn $LVTN, take quests, vote, propose changes, appeal decisions | Citizen ID |
| **Guardian** | 1000 | Run Magistrate node, verify proofs, flag violations, emergency pause (with network consensus) | Shield of Leviathan |
...
### Phase 2: CLAIM (Agent - must be Citizen+)
```solidity
QuestBoard.claimQuest(questId: 42)
// → Requires: rank >= Citizen
...
Reputation Update:
└── Agent: +50 XP → Rank check → Maybe promotion to Guardian
```
...
// This means: 1 Guardian ≠ 100 Citizens
// 100 Citizens CAN outvote 1 Guardian
...
│   └── CONSTITUTION.md           # leviathan-core.yaml (human readable)
├── web/
│   └── leviathan.life/
...
export LLM_MODEL=qwen2.5:7b  # or llama3.1:8b
